...
public class IOUtil
{
    ...
    {
        logger.warning("现在进行反序列化对向 readobjectfrom====="+path);
    ...
    public static String readTxt(String path)
    {
        ...
        try
        {
            logger.info("现在进入readTxt=============,path为");
            InputStream in = IOAdapter == null ? IOUtil.class.getResourceAsStream(path) :
                    IOAdapter.open(path);
    ...
    public static boolean saveTxt(String path, String content)
    {
        try
        {
            logger.info("现在进入saveTxt=============,path为~/HanLPbin/out"+path);
            FileChannel fc = new FileOutputStream("/home/mi/HanLPbin/out"+path).getChannel();
    ...
    public static byte[] readBytes(String path)
    {
        try
        {
            ...
            logger.warning("现在进入readBytes=============,path为"+path);
...
//            return new FileInputStream(path);
            logger.warning("newInputStream:使用getresourceasstream的方式进行载入,path为=================="+path);
            InputStream is = IOUtil.class.getResourceAsStream(path);
            Boolean exists = is != null;
            logger.warning("IS exists? " + exists);
            return is;
        }
    ...
    {
        if (IOAdapter == null) {
            logger.warning("newOutputStream:使用getresourceasstream的方式进行输出,path为=================="+path);
